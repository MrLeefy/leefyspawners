...
import { forceShowForm } from "./ui-utils.js";
import { configDatabase, clearMaxedSpawnerCache } from "./mobstacker-core.js";
...
function encodeCoordinates(x, y, z) {
    const shift = 3;
...
function decodeCoordinates(encodedString) {
...
function createSpawnerForm(player, level, upgradee, downgradee, spawnerType, block, typeId, percentrefund, refu, x, y, z, coordinates) {
    ...
    let bodyText = `§7§l\n                §7Level: §2${level}§r\n\n`;
    // Append Chest Link Status to Form Body
    try {
        ...
        if (spawnerData && spawnerData.linkedChest) {
            ...
            let statusLines = `§7Linked Chest: §a${chest.x}, ${chest.y}, ${chest.z}\n`;
            try {
                const chestDim = world.getDimension(chest.dimensionId);
                const chestBlock = chestDim?.getBlock({ x: chest.x, y: chest.y, z: chest.z });
                if (!chestBlock) {
                    statusLines += `§8Status: §cUnloaded Chunk\n`;
                }
                else {
                    const containers = getDoubleChestContainers(chestBlock);
                    const container1 = containers.container1;
                    const container2 = containers.container2;
                    if (!container1) {
                        statusLines += `§8Status: §cMissing / Broken\n`;
                    }
                    else {
                        let occupiedSlots = 0;
                        let totalSize = container1.size;
                        for (let i = 0; i < container1.size; i++) {
                            if (container1.getItem(i) !== undefined) {
                                occupiedSlots++;
                            }
                        }
                        if (container2) {
                            totalSize += container2.size;
                            for (let i = 0; i < container2.size; i++) {
                                if (container2.getItem(i) !== undefined) {
                                    occupiedSlots++;
                                }
                            }
                        }
                        const pct = Math.round((occupiedSlots / totalSize) * 100);
                        // 1. Color-coded progress bar based on percentage
                        let colorCode = "§a"; // Green
                        if (pct >= 90) {
                            colorCode = "§c"; // Red (Critical)
                        }
                        else if (pct >= 75) {
                            colorCode = "§e"; // Yellow (Warning)
                        }
                        const filledBlocks = Math.min(10, Math.max(0, Math.round(pct / 10)));
                        const emptyBlocks = 10 - filledBlocks;
                        const bar = colorCode + "█".repeat(filledBlocks) + "§8" + "█".repeat(emptyBlocks);
                        // 2. Exact container type and size display
                        let cleanType = chestBlock.typeId
                            .replace("minecraft:", "")
                            .split("_")
                            .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
                            .join(" ");
                        if (container2) {
                            cleanType = "Double " + cleanType;
                        }
                        else if (cleanType === "Chest") {
                            cleanType = "Single " + cleanType;
                        }
                        statusLines += `§7Type: §e${cleanType} §8(${totalSize} Slots)\n`;
                        statusLines += `§7Fullness: §7[${bar}§7] ${colorCode}${pct}%% §7Full\n`;
                    }
                }
            }
            catch (e) {
                statusLines += `§8Status: §cOffline\n`;
            }
            bodyText += statusLines + `\n`;
        }
    }
    catch (e) {
        console.error("Error adding chest status to form body:", e);
    }
    form1.body(bodyText);
    const buttonActions = [];
    ...
    // Spawner Chest Linking System Button
    try {
        const spawnerData = spawnerDatabase.read(coordinates);
        if (spawnerData && spawnerData.linkedChest) {
            form1.button(`§l§cUnlink Chest`, 'textures/items/minecart_chest');
            buttonActions.push(() => unlinkChest(player, coordinates));
        }
        else {
            form1.button(`§lLink Chest`, 'textures/items/minecart_chest');
            buttonActions.push(() => startChestLinking(player, coordinates));
        }
    }
    catch (e) {
        console.error("Error adding Link Chest button to UI:", e);
    }
    ...
    if (player.hasTag(UI.OWNER_PERMISSION_TAG)) {
        ...
        const chooseLevelAction = () => slider(player, spawnerType, block, level, 10000 * level, typeId, upgradee, downgradee, percentrefund, refu, x, y, z);
...
function form1(player, level, cost, block, typeId, upgradee, downgradee, percentrefund, refu, spawnerType, x, y, z) {
    ...
    // Trigger particle link wave to show player where the spawner is linked
    try {
        const spawnerData = spawnerDatabase.read(coordinates);
        if (spawnerData && spawnerData.linkedChest) {
            const chest = spawnerData.linkedChest;
            ...
            if (chestBlock && chestBlock.getComponent('inventory')?.container) {
                triggerChestLinkParticles(player, x, y, z, chest.x, chest.y, chest.z, chest.dimensionId);
            }
        }
    }
    catch (e) {
        console.error("Error triggering particles on spawner menu open:", e);
    }
...
function teleportSpawnerStack(player, block, spawnerType, x, y, z) {
    ...
    for (const entity of nearbyEntities) {
        try {
            if (!entity || !entity.isValid)
                continue;
            const distance = Math.sqrt(Math.pow(entity.location.x - x, 2) +
    ...
    if (closestEntity) {
        ...
        const centerY = y + 1;
        const centerZ = z + 0.5;
        closestEntity.teleport(new Vector3(centerX, centerY, centerZ), { keepVelocity: true });
...
function slider(player, spawnerType, block, level, cost, typeId, upgradee, downgradee, percentrefund, refu, x, y, z) {
    ...
    forceShowForm(player, slider).then((response) => {
...
function showInstructions(player) {
    ...
    instructions.body('§f§lGetting Started§r\n' +
        ...
        '§d§lChest Linking (QOL)§r\n' +
        '§7- Tap §dLink Chest§7 and right-click a nearby chest\n' +
        '§7- Enforces a §e15-block max distance§7 in the same dimension\n' +
        '§7- Automatically routes stack loot drops into the chest\n' +
        '§7- Supports double chests (54 slots) and single chests/hoppers\n' +
        '§7- Shows progress bar & orbits chest with portal particles\n' +
        '§7- Sneak (crouch) while in link mode to cancel linking\n\n' +
        '§e§lTeleport Stack§r\n' +
...
function maxUpgradeSpawner(player, block, level, spawnerType, typeId, x, y, z) {
    ...
    try {
        block.dimension.spawnParticle('minecraft:crop_growth_area_emitter', new Vector3(block.x + 0.5, block.y + 0.5, block.z + 0.5), new MolangVariableMap());
    }
    catch (e) { }
...
function downgrade(player, block, level, spawnerType, percentrefund, refu, x, y, z) {
    ...
    try {
        dimension.spawnParticle('minecraft:villager_angry', new Vector3(block.x + 0.5, block.y + 0.5, block.z + 0.5), new MolangVariableMap());
    }
    catch (e) { }
}
