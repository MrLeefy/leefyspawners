...
import { TIMING, UI, ENTITIES, VALIDATION } from "./constants.js";
...
// Track entities in their death animation (health = 0 but still in world)
// Prevents spawner loop from double-counting them alongside the replacement entity
...
export function extractStackNumber(nameTag) {
...
export function clearSpawnerParseCache() {
...
function getSpawnerSpecs(nameTag) {
    ...
    if (specs !== undefined)
        return specs;
    const parsedName = nameTag.replace(/(_)|(spawner)/gi, (match) => match === '_' ? "" : match.toLowerCase() === 'spawner' ? 'still' : "");
        ...
        for (const spawnruleEntity of spawnruleEntities) {
            ...
            for (const entity of nearbyEntities) {
                if (!entity || !entity.isValid)
                    continue;
                if (dyingEntities.has(entity.id))
                    continue; // skip death-animation entities
...
// Helper function to extract Reaper level from a weapon's dynamic properties or lore
function getReaperLevel(item) {
    if (!item)
        return 0;
    // 1. Check Dynamic Property
    try {
        const prop = item.getDynamicProperty("reaper");
        if (typeof prop === "number" && prop >= 1 && prop <= 5) {
            return prop;
        }
    }
    catch (e) { }
    ...
    try {
        ...
        if (lore && lore.length > 0) {
            for (const line of lore) {
                const cleanLine = line.replace(/§./g, "").trim(); // Strip color codes
                const match = cleanLine.match(/^Reaper\s+(I|II|III|IV|V)$/i);
                if (match) {
...
if (!globalThis.__stackDieSubscribed) {
    ...
    world.afterEvents.entityHurt.subscribe((event) => {
        try {
            ...
            // Entity is dying — mark it so the spawner loop ignores it during the death animation
            // runTimeout cleans it up after 5 ticks (death animations are ~3-4 ticks in Bedrock)
            const _dyingId = hurtEntity.id;
            dyingEntities.add(_dyingId);
            system.runTimeout(() => { dyingEntities.delete(_dyingId); }, 5);
            ...
            const killerPlayer = killer?.typeId === "minecraft:player" ? killer : undefined;
                ...
                try {
                    ...
                    if (killerPlayer) {
                        try {
                            ...
                            if (mainhand) {
                                // Check for custom Reaper enchantment - overrides baseReap
                                const reaperLevel = getReaperLevel(mainhand);
                                if (reaperLevel > 0) {
                                    const reaperAmounts = [0, 3, 6, 12, 18, 30];
                                    reaperBonus = reaperAmounts[reaperLevel];
                                }
                    ...
                    const effectiveReap = reaperBonus > 0 ? reaperBonus : baseReap;
                    const lootingBonus = getCachedConfig("reapLootingBonus", true) ? lootLevel * 2 : 0;
                    totalReaped = Math.min(currentAmount, effectiveReap + lootingBonus);
                    ...
                    // Set spawner key for chest linking (if linked)
                    if (inheritedSpawnerKey) {
                        deadEntitySpawnerMap.set(hurtEntity.id, inheritedSpawnerKey);
                    }
