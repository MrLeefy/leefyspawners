...
class LootManager {
        ...
        try {
            ...
            for (const entityId of savedKeys) {
                ...
                if (savedLootTable && Object.keys(savedLootTable).length > 0) {
                    ...
                    if (hasInvalidBeds) {
                        savedLootTable['minecraft:bed'] = {
                            chance: (savedLootTable['minecraft:bed']?.chance ?? 0) + bedChanceAcc,
                            stackable: false
                        };
                    ...
                    if (savedLootTable['minecraft:item_frame']) {
                        ...
                        delete savedLootTable['minecraft:item_frame'];
                    ...
                    if (savedLootTable['minecraft:terracotta']) {
                        ...
                        delete savedLootTable['minecraft:terracotta'];
     ...
     * @param singleDrop Whether to select exactly one drop (for villagers)
        ...
        if (singleDrop) {
            ...
            for (const [itemId, count] of Object.entries(pickedCounts)) {
                ...
                for (let s = 0; s < count; s++) {
                    ...
                    totalQuantity += dropQuantity;
                }
                finalLoot[itemId] = { ...config, quantity: totalQuantity };
            }
            return finalLoot;
        }
     ...
     * @param lootLevel - The level of Looting enchantment
     * @param reapMultiplier - The reap multiplier
    ...
    processLootDrops(finalLoot, dimension, location, lootLevel = 0, reapMultiplier = 1, spawnerKey, entityId) {
            ...
            try {
                ...
                if (spawnerData && spawnerData.linkedChest) {
                    ...
                    if (chestDim) {
                        ...
                        if (block) {
                            const containers = getDoubleChestContainers(block);
                            container1 = containers.container1;
                            container2 = containers.container2;
                        }
                    }
                }
            }
        ...
        for (const { itemId, config } of stackableItems) {
            try {
                let remaining = config.quantity ?? 1;
                while (remaining > 0) {
...
export function getDoubleChestContainers(block) {
    ...
    if (typeId.includes('chest') && !typeId.includes('ender_chest')) {
        ...
        for (const offset of offsets) {
            try {
                const neighbor = dimension.getBlock({ x: block.x + offset.x, y: block.y, z: block.z + offset.z });
                if (neighbor && neighbor.typeId === typeId) {
                    const container2 = neighbor.getComponent('inventory')?.container;
                    if (container2) {
