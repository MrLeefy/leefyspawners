...
world.afterEvents.playerPlaceBlock.subscribe(event => {
    ...
    let entityType = "mrleefy:spawnrule";
    ...
    system.runTimeout(() => {
        let allEntities = dimension.getEntities({
            maxDistance: 50,
            location: event.block.location,
            type: entityType
        });

        let entityAtBlockLocation = allEntities.find(entity => Vector3.Distance(entity.location, event.block.location) <= 1.0);

        if (entityAtBlockLocation) {
            let entitiesWithinRadius = allEntities.filter(entity => Vector3.Distance(entity.location, event.block.location) <= 50 && Vector3.Distance(entity.location, entityAtBlockLocation.location) > 0.5);
            if(event.block.typeId.includes('spawner')) {
            let eventBlockNameTag = event.block.typeId.split(':')[1].replace(/\d+$/, '');
            let entityNameTag = entityAtBlockLocation.nameTag.split(' ')[0].split(':')[1].replace(/\d+/g, '').replace(/,$/, '');

            entitiesWithinRadius.forEach(entity => {
                let nameTagWithoutCoords = entity.nameTag.split(' ')[0].split(':')[1].replace(/\d+/g, '').replace(/,$/, '');
