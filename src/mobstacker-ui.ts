...
import { world, system, Player, Block, Entity, Vector3, Dimension, CommandPermissionLevel, CustomCommandParamType, CustomCommandStatus, CustomCommand, CustomCommandOrigin, CustomCommandResult, StartupEvent } from "@minecraft/server";
    ...
    try {
        ...
        form.show(player).then((response: any) => {
            ...
            if (response.selection === 2) {
                 const confirmForm = (new ModalFormData() as any)
                    .title("Confirm Reset")
...
export const playerLinkingState = new Map<string, { spawnerKey: string, dimensionId: string }>();
 ...
 */
export function startChestLinking(player: Player, spawnerKey: string): void {
 ...
 */
export function unlinkChest(player: Player, spawnerKey: string): void {
    const spawnerData = spawnerDatabase.read(spawnerKey);
    if (!spawnerData) {
        player.sendMessage("§c[Spawner Link] Error: Spawner not found in database.");
        return;
...
const activeParticleIntervals = new Map<string, { intervalId: number; elapsedTicks: number }>();
 ...
 */
export function triggerChestLinkParticles(
    player: Player,
    spawnerX: number,
    spawnerY: number,
    spawnerZ: number,
    ...
    let spawnerTypeId: string | undefined = undefined;
    try {
        const spawnerData = spawnerDatabase.read(spawnerKey);
