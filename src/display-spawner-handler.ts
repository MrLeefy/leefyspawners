...
function getPlayerMoney(player) {
    try {
        ...
        if (moneyObjective) {
            ...
            // If player doesn't have a score, add them with 0
...
world.afterEvents.playerSpawn.subscribe((event) => {
    try {
        ...
        // Add player to scoreboard by adding 0 (doesn't change their score, just registers them)
        system.run(() => {
            try {
                ...
                // Player might already be on scoreboard, which is fine
