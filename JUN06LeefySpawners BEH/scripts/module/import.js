...
world3.beforeEvents.playerInteractWithBlock.subscribe((data) => {
  ...
  if (!spawnerDatabase.has(coordinates)) {
    player.sendMessage("\xA7cSomething went wrong, please pick up this spawner and place it back down.");
    return;
  }
  system2.run(() => {
...
`;
    bodyText += `\xA7bSpawner Blocks Placed: \xA7f${spawnerStatistics.totalSpawners.toLocaleString()}
`;
    bodyText += `\xA7bEntities Spawned (Total): \xA7f${spawnerStatistics.totalEntities.toLocaleString()}
`;
    bodyText += `\xA7bActive Players: \xA7f${activePlayers}
`;
    bodyText += `\xA7bChunks Loaded (Spawner Activity): \xA7f${ACTIVE_CHUNKS.size}
`;
    bodyText += `\xA7bServer Load: \xA7f${serverLoad}%
`;
    bodyText += `\xA7bTotal Kills: \xA7f${totalKillsFormatted}
`;
    bodyText += `\xA7bServer Uptime: \xA7f${uptimeDisplay}
`;
    bodyText += `\xA7bMemory Usage: \xA7f${memoryLevel} (${memoryUsage.toLocaleString()} units)
`;
    bodyText += `\xA7bTick Usage: \xA7f${tickEfficiency}% (${performanceMetrics.averageProcessingTime.toFixed(2)}ms)
...
`;
    ...
    const form = new ActionFormData2().title("\xA78Spawner Server Statistics").body(bodyText).button("\xA7cClose").button("View Player Stats").button("Reset All Statistics");
    form.show(player).then((response) => {
      if (!securityService.hasTagPermission(player, UI.ADMIN_PERMISSION_TAG)) {
        player.sendMessage(ERROR_MESSAGES.NO_PERMISSION);
        return;
...
`;
    spawnerDetails.sort((a, b) => b.activeEntities - a.activeEntities).slice(0, 5).forEach((spawner, index) => {
...
system4.runInterval(() => {
  try {
    ...
    saveSpawnerStatistics2();
  } catch (error) {
    console.error("Error in persistent statistics sync:", error);
  }
}, 30 * 20);
function saveSpawnerStatistics2() {
  try {
    const statsObj = {
      entitiesKilled: Array.from(spawnerStatistics.entitiesKilled.entries()),
      spawnerUptime: Array.from(spawnerStatistics.spawnerUptime.entries()),
      playerStats: Array.from(spawnerStatistics.playerStats.entries()),
...
function calculateSpawnerTotals() {
  ...
  spawnerStatistics.totalSpawners = spawnruleEntities.length;
  let totalEntities = 0;
  const validMobs2 = [
    "mrleefy:blazestill",
    "mrleefy:cowstill",
  ...
  spawnerStatistics.totalEntities = totalEntities;
  spawnerStatistics.lastStatsUpdate = Date.now();
}
...
function* spawnerProcessingJob() {
  try {
    ...
    performanceMetrics.averageProcessingTime = (performanceMetrics.averageProcessingTime + processingTime) / 2;
  } catch (error) {
...
function getPlayerMoney(player) {
  try {
    ...
    if (moneyObjective) {
      ...
      if (score === void 0 || score === null) {
        try {
          ...
          console.warn(`[Display Spawner] Registered ${player.name} on scoreboard with 0`);
        } catch (e) {
        }
        return 0;
      }
    ...
    console.warn(`[Display Spawner] Scoreboard objective '${objectiveName}' not found!`);
    return 0;
  } catch (error) {
    console.warn(`[Display Spawner] Error getting player money: ${error}`);
    return 0;
  }
}
function removePlayerMoney(player, amount) {
  try {
    ...
    try {
      const objective = world7.scoreboard.getObjective(getMoneyObjective());
      if (objective) {
...
world7.afterEvents.entityHitEntity.subscribe((event) => {
  ...
  if (!hitEntity || !hitEntity.typeId.endsWith("still_display")) {
    return;
  }
  try {
...
world7.afterEvents.playerSpawn.subscribe((event) => {
  try {
    ...
    if (!objective) {
      console.warn(`[Display Spawner] Scoreboard objective ${moneyObjective} not found!`);
      return;
    }
    system7.run(() => {
      try {
        const currentScore = objective.getScore(player) ?? 0;
        objective.setScore(player, currentScore + 0);
        console.warn(`[Display Spawner] Added ${player.name} to scoreboard ${moneyObjective}`);
      } catch (error) {
        console.warn(`[Display Spawner] ${player.name} already on scoreboard or error: ${error}`);
      }
    });
  } catch (error) {
